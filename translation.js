const { sign } = require("jsonwebtoken");

const translations = {
  en: {
    season: { Rabi: "Rabi", Kharif: "Kharif", Summer: "Summer" },
    stage: {
      Sowing: "Sowing",
      Vegetative: "Vegetative",
      Flowering: "Flowering",
      Harvest: "Harvest"
    },
    type: { Fertilizer: "Fertilizer", Pesticide: "Pesticide" },
    soil_type: { Black: "Black", Red: "Red", Sandy: "Sandy", All: "All" },
    pages_name: {
      Home: "Home",
      ABOUT_US: "ABOUT US",
      Product: "Product",
      Farming_Tips: "Farming Tips",
      Soil_Testing: "Soil Testing",
      Recomendation: "Recomendation",
      Goverment_Schemes: "Goverment Schemes",
      CROPS: "CROPS",
      CROP_MANAGEMENT: "CROP MANAGEMENT",
      SOIL_HEALTH: "SOIL HEALTH",
      WEATHER: "WEATHER",
      GALLERY: "GALLERY",
      CONTACT_US: "CONTACT US",
    },
    buttons: {Register:"Register",Call_To_Order:"Call to Order",
      Call_us:"Call Us",
      Follow_us:"Follow Us",
      next:"Next",
      forgot:"Forgot Password?",
      previous:"Previous",
      register:"Register",
        Profile:"My Profile",
        logout:"Logout",
        in_stock:"In Stock",
        low_stock:"Low Stock",
        add_to_cart: "Add to Cart",
        save :"Save",
        Categories: "Categories",
        brands: "Brands",
        price_range: "Price Range",
        customer_rating: "Customer Rating",

    },
    footer: {about_company:"Krushi Seva Kendra is your trusted partner for agricultural products, seeds, fertilizers, and expert farming guidance to boost your productivity.",
       quick_links: "Quick Links",
      home: "Home",  
      about: "About Us",
      products: "Products",
      services: "Services",
      contact: "Contact Us",
      Customer_support: "Customer Support",
      Faq:"FAQ",
      Shipping_Delivery: "Shipping & Delivery",
      Returns_Policy: "Returns Policy",
      Terms_condtion: "Terms & Conditions",
      copyright:"Krushi Seva Kendra. All Rights Reserved."
    },
    account:{your_account:"Create Your Account",
      join_us:"Join Krushi Seva Kendra in just 2 easy steps",
      name:"Full Name",
      email:"Email Address",
      mobile:"Mobile Number",
      password:"Password",
      already_account:"Already have an account?",
      sign_in:"Sign In",
      address:"Address",
      crops:"Selected Crops",
      welcome:"Welcome Farmer",
      signin:"Sign in to continue to Krushi Seva Kendra",
      dont_account:"Don't have an account?",
      signup:"Sign Up",

    }

  },
  mr: {
    season: { Rabi: "रबी", Kharif: "खरीप", Summer: "हिवाळी" },
    stage: {
      Sowing: "पेरणी",
      Vegetative: "वाढ",
      Flowering: "फुलं येणे",
      Harvest: "पिक काढणी"
    },
    type: { Fertilizer: "खत", Pesticide: "किडीमारा" },
    soil_type: { Black: "काळा", Red: "लाल", Sandy: "वाळू", All: "सर्व" },
    pages_name: {
     Home: "मुख्यपृष्ठ",
ABOUT_US: "आमच्याबद्दल",
Product: "उत्पादने",
Farming_Tips: "शेतीविषयक टिप्स",
Soil_Testing: "माती परीक्षण",
Recomendation: "शिफारसी",
Goverment_Schemes: "शासकीय योजना",

      WEATHER: "हवामान",
      GALLERY: "गॅलरी",
      CONTACT_US: "संपर्क",
      ABOUT_US: "आमच्याबद्दल"
    },

    buttons: {Register:"नोंदणी करा",Call_To_Order:"ऑर्डर करण्यासाठी कॉल करा",
       Call_us: "आम्हाला कॉल करा",
  Follow_us: "आमचे अनुसरण करा",
   next: "पुढे",
    forgot: "संकेतशब्द विसरलात?",
     previous: "मागील",
  register: "नोंदणी करा",
    Profile: "माझे प्रोफाइल",
  logout: "लॉगआऊट",
  in_stock: "स्टॉकमध्ये आहे",
low_stock: "कमी स्टॉक",
add_to_cart: "कार्टमध्ये जोडा",
save: "जतन करा",
Categories: "वर्गीकरण",
brands: "ब्रँड",
price_range: "किंमत श्रेणी",
customer_rating: "ग्राहक रेटिंग"



    },
    footer: {about_company:"कृषी सेवा केंद्र हे कृषी उत्पादने, बियाणे, खते आणि तुमची उत्पादकता वाढवण्यासाठी तज्ञ शेती मार्गदर्शनासाठी तुमचा विश्वासू भागीदार आहे.",
       quick_links: "जलद दुवे",
      home: "मुख्यपृष्ठ",  
      about: "आमच्याबद्दल",
      products: "उत्पादने",
      services: "सेवा",
      contact: "संपर्क",
 Customer_support: "ग्राहक समर्थन",
  Faq: "सामान्य प्रश्न",
  Shipping_Delivery: "शिपिंग आणि वितरण",
  Returns_Policy: "परतावा धोरण",
  Terms_condtion: "नियम आणि अटी",
  copyright: "कृषी सेवा केंद्र. सर्व हक्क राखीव आहेत."

    },
     account : {
  your_account: "आपले खाते तयार करा",
  join_us: "फक्त 2 सोप्या टप्प्यात कृषी सेवा केंद्रात सामील व्हा",
  name: "पूर्ण नाव",
  email: "ईमेल पत्ता",
  mobile: "मोबाईल क्रमांक",
  password: "संकेतशब्द",
  already_account: "आधीच खाते आहे का?",
  sign_in: "साइन इन करा",
   address: "पत्ता",
  crops: "निवडलेले पीक",
    welcome: "स्वागत आहे शेतकरी",
  signin: "कृषी सेवा केंद्रात पुढे जाण्यासाठी साइन इन करा",
  dont_account: "अजून खाते नाही का?",
  signup: "साइन अप करा"
}


  },
  hi: {
    season: { Rabi: "रबी", Kharif: "खरीफ", Summer: "ग्रीष्म" },
    stage: {
      Sowing: "बुवाई",
      Vegetative: "वृद्धि",
      Flowering: "फूलना",
      Harvest: "कटाई"
    },
    type: { Fertilizer: "उर्वरक", Pesticide: "कीटनाशक" },
    soil_type: { Black: "काला", Red: "लाल", Sandy: "रेतीला", All: "सभी" },
    pages_name: {
      Home: "मुखपृष्ठ",
ABOUT_US: "हमारे बारे में",
Product: "उत्पाद",
Farming_Tips: "खेती के सुझाव",
Soil_Testing: "मृदा परीक्षण",
Recomendation: "सिफारिशें",
Goverment_Schemes: "सरकारी योजनाएँ",
      WEATHER: "मौसम",
      GALLERY: "गैलरी",
      CONTACT_US: "संपर्क करें",
      ABOUT_US: "हमारे बारे में"
    },
        buttons: {Register:"पंजीकरण करवाना",Call_To_Order:"आदेश करने के लिए फोन करें",
           Call_us: "हमें कॉल करें",
  Follow_us: "हमें फॉलो करें",
   next: "अगला",
    forgot: "पासवर्ड भूल गए?",
     previous: "पिछला",
  register: "पंजीकरण करें",
  Profile: "मेरा प्रोफाइल",
  logout: "लॉगआउट",
  in_stock: "स्टॉक में है",
low_stock: "कम स्टॉक",
add_to_cart: "कार्ट में जोड़ें",
save: "सहेजें",
Categories: "श्रेणियाँ",
brands: "ब्रांड",
price_range: "कीमत सीमा",
customer_rating: "ग्राहक रेटिंग"



        },
    footer: {about_company:"कृषि सेवा केंद्र आपकी उत्पादकता बढ़ाने के लिए कृषि उत्पादों, बीजों, उर्वरकों और विशेषज्ञ कृषि मार्गदर्शन के लिए आपका विश्वसनीय भागीदार है।",
      quick_links: "त्वरित लिंक",
      home: "होम",  
      about: "हमारे बारे में",
      products: "उत्पाद",
      services: "सेवाएं",
      contact: "संपर्क",
 Customer_support: "ग्राहक समर्थन",
  Faq: "सामान्य प्रश्न",
  Shipping_Delivery: "शिपिंग और डिलीवरी",
  Returns_Policy: "वापसी नीति",
  Terms_condtion: "नियम और शर्तें",
  copyright: "कृषि सेवा केंद्र. सर्वाधिकार सुरक्षित।"

    },
     account : {
  your_account: "अपना खाता बनाएं",
  join_us: "सिर्फ 2 आसान चरणों में कृषि सेवा केंद्र से जुड़ें",
  name: "पूरा नाम",
  email: "ईमेल पता",
  mobile: "मोबाइल नंबर",
  password: "पासवर्ड",
  already_account: "क्या पहले से खाता है?",
  sign_in: "साइन इन करें",
   address: "पता",
  crops: "चुने हुए फ़सल",
   welcome: "स्वागत है किसान",
  signin: "कृषि सेवा केंद्र में आगे बढ़ने के लिए साइन इन करें",
  dont_account: "खाता नहीं है?",
  signup: "साइन अप करें"
}



  }
};

module.exports = translations;
